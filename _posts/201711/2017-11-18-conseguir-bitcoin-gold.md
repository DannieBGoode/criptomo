...
**Si tuvieramos el seed del wallet pero no la clave privada** de la dirección específica podemos obtener la clave privada usando la herramienta [Bip39](https://github.com/iancoleman/bip39). Descargamos esta herramienta de github y abrimos el fichero `bip39-standalone.html` en cualquier navegador. En esta herramienta meteriamos el seed en el campo: `BIP39 Mnemonic` y seleccionaremos `Coin: BTC - Bitcoin` (que no BTG puesto que la dirección original es de BTC) y `Derivation Path: BIP44`. Dejaremos las demás opciones como están. Abajo del todo en `Derived Addresses` deberíamos ver todas las direcciones de nuestro wallet con su clave pública y privada. Nuestra dirección debería estar entre las primeras posiciones.
